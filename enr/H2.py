...
#from SMPL.smplpytorch.pytorch.smpl_layer import SMPL_Layer
#smpl_layer = SMPL_Layer(center_idx=0,gender='neutral',model_root='smplpytorch/native/models').to(dtype=torchdtype, device=torchdeviceId)
##############################################################################################################################
#H2 Helper Functions
##############################################################################################################################
...
def enr_match_H2_SMPL(VS,FS, F_sol, Fun_Sol, X_total, weight_coef_dist_T=1, weight_Gab=1, a0=1, a1=1, b1=1, c1=1, d1=1, a2=1, **objfun):    
    FunS = torch.from_numpy(np.zeros((int(np.size(VS.cpu().numpy()) / 3),))).to(dtype=torchdtype, device=torchdeviceId)
    K=VKerenl(objfun['kernel_geom'],objfun['kernel_grass'],objfun['kernel_fun'],objfun['sig_geom'],objfun['sig_grass'],objfun['sig_fun'])
    dataloss = lossVarifoldSurf(F_sol, Fun_Sol, VS, FS, FunS, K)  
    def energy(X):
        X=torch.cat((torch.unsqueeze(torch.zeros((X.shape[1])).to(dtype=torchdtype, device=torchdeviceId),dim=0),X),dim=0).requires_grad_(True)
        geod_torch, jtr = smpl_layer(X[:,:72], th_betas=X[:,72:372], th_trans=X[:,372:375])
        N = geod_torch.shape[0]
        enr = getPathEnergyH2(geod_torch, a0, a1, b1, c1, d1, a2, F_sol)
        return weight_Gab * enr +  weight_coef_dist_T*dataloss(geod_torch[-1])
    return energy

def enr_param_H2_SMPL( F_sol, X_total, a0=1, a1=1, b1=1, c1=1, d1=1, a2=1, **objfun):
    def energy(X):
        X_total[1:-1]=X_total[1:-1]+X
        geod_torch, jtr = smpl_layer(X_total[:,:72], th_betas=X_total[:,72:372], th_trans=X_total[:,372:375])
